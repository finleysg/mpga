...
const DocumentActions = {
  ...
  Load: (query: IDocumentSearch) => async (dispatch: any) => {
    ...
    try {
      ...
      if (result && result.data) {
        const data = result.data as IDocumentData[];
        dispatch({
  ...
  Save:
    (key: string, document: MpgaDocument, file?: File) => async (dispatch: any, getState: () => IApplicationState) => {
      const queries = getState().documents.queries;
      const requery = Object.prototype.hasOwnProperty.call(queries, key) && queries[key];
      dispatch(AppActions.Busy());
      try {
