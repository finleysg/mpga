...
import { IRegisterData, UserActionTypes } from "./UserActions";
...
export const UsersReducer: Reducer<IUserState, KnownActions> = (
  ...
  switch (action.type) {
    ...
    case UserActionTypes.GET_CONTACT_ROLES_SUCCEEDED: {
      const committeeId = action.payload.committee?.length > 0 ? action.payload.committee[0].id : undefined;
      const clubId = action.payload.club?.length > 0 ? action.payload.club[0].club__system_name : undefined;
      return { ...state, clubId, committeeId };
    }
    ...
    case UserActionTypes.RESET_USER: {
      return { ...state, user: defaultState.user, flags: defaultStateFlags };
    }
